...
- **Positional neglect** — attention is not uniform across context. The "lost in the middle" effect (Liu et al., 2023) demonstrated a U-shaped retrieval curve in earlier models: information at the beginning and end of context was well-attended, while mid-context information was under-weighted. Modern long-context architectures (Claude 3+/4, etc.) have specifically optimized for more uniform attention and show significantly reduced positional bias — the effect is negligible in short-to-medium contexts and most pronounced in very long contexts (100k+ tokens) on pure retrieval tasks rather than synthesis or reasoning. It remains a diminishing but real concern. Separately, during long *generation*, attention sinks (disproportionate weight on initial tokens) combine with trajectory commitment to progressively narrow the effective context the model draws from. Both patterns matter for pipeline design: the first informs how to arrange information within a stage's context window when working near capacity limits; the second constrains how long a single generation pass can run before context utilization degrades
