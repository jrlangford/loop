...
- **Claude Code plugin (Loop structure)**: Look for `.claude-plugin/plugin.json` — this indicates a plugin, likely generated by `/loop:implement`. Inside, look for `skills/<prefix>/` containing `stages/` (stage reference documents) and `contracts/` (artifact schemas). Orchestrator skills (`skills/run/SKILL.md` or `skills/run-<workflow>/SKILL.md`) sequence stages and manage gates/loops. Stage boundaries are the files in `stages/`. Artifacts are defined by the files in `contracts/`. Note the plugin `name` from `plugin.json` — it's the pipeline prefix.
- **Claude Code skills (shared resource structure, no plugin)**: Look for a `<prefix>/` directory containing `stages/` and `contracts/` without plugin packaging. Orchestrator skills (`<prefix>-run/SKILL.md`) sequence stages and manage gates/loops. This is an older structure — note the absence of plugin packaging in synthesis notes.
